...
static long getcard(Window w, Atom prop, long def) {
    ...
    if (XGetWindowProperty(dpy, w, prop, 0L, 1L, False, XA_CARDINAL, &real,
                           &fmt, &n, &extra, &p) == Success &&
        p) {
        if (n)
            v = *(long *)p;
        XFree(p);
...
static void drawthumb(Thumb *t, float s, int issel) {
    ...
    if (t->pict) {
        ...
        if (t->dst.height > font->ascent + font->descent + 4)
            drawtext(t->dst.x + 4,
                     t->dst.y + (t->dst.height + font->ascent) / 2, fgpx,
                     t->title);
    }
...
static int rowat(int y) {
    ...
    for (i = 0; i < nrows; i++)
        if (y >= rows[i].y - rowgap / 2 &&
            y < rows[i].y + rows[i].h + rowgap / 2)
            return i;
    return -1;
}
...
static void keypress(XKeyEvent *ev) {
    ...
    if (sym == XK_Escape || sym == XK_q) {
        if (initialactive)
            sendroot(netactivewindow, initialactive,
                     2); /* undo focus browsing */
        running = 0;
        return;
    }
    switch (sym) {
...
static void handle(XEvent *ev) {
    ...
    case ConfigureNotify: /* a source window moved or resized */
        ...
        if (i >= 0) {
            ...
            thumbs[i].w =
                ev->xconfigure.width + 2 * ev->xconfigure.border_width;
            thumbs[i].h =
                ev->xconfigure.height + 2 * ev->xconfigure.border_width;
    ...
    case PropertyNotify:
        if (ev->xproperty.window == root) {
            ...
            if (i >= 0) {
                gettitle(thumbs[i].win, thumbs[i].title,
                         sizeof(thumbs[i].title));
                dirty = 1;
            }
        }
        break;
...
static void setup(void) {
    ...
    overlay =
        XCreateWindow(dpy, root, mx, my, (unsigned int)mw, (unsigned int)mh, 0,
                      CopyFromParent, CopyFromParent, CopyFromParent,
                      CWOverrideRedirect | CWBackPixel | CWEventMask, &swa);
    ch.res_name = ch.res_class = "hws"; /* compositor rules match on class */
    XSetClassHint(dpy, overlay, &ch);
